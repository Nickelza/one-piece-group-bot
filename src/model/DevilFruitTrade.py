...
class DevilFruitTrade(BaseModel):
    ...
    """
    id = PrimaryKeyField()
    devil_fruit = ForeignKeyField(DevilFruit, null=True, backref='devil_fruit_trades', on_delete='RESTRICT',
                                  on_update='CASCADE')
    giver = ForeignKeyField(User, null=True, backref='devil_fruit_trade_givers', on_delete='RESTRICT',
                            on_update='CASCADE')
    receiver = ForeignKeyField(User, null=True, backref='devil_fruit_trade_receivers', on_delete='RESTRICT',
                               on_update='CASCADE')
    ...
    group_chat = ForeignKeyField(GroupChat, null=True, backref='devil_fruit_trade_group_chats', on_delete='RESTRICT',
                                 on_update='CASCADE')
