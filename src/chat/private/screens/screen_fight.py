...
async def manage(
    ...
    if not await fight_validate(update, context, user, False, inbound_keyboard):
        return

    return await private_manage(update, context, user, inbound_keyboard, ScoutType.FIGHT)
