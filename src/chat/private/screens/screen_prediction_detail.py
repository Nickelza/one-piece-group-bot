...
from src.service.prediction_service import user_has_bet_on_prediction, delete_prediction, get_share_text, close_bets
...
async def manage(update: Update, context: ContextTypes.DEFAULT_TYPE, inbound_keyboard: Keyboard, user: User) -> None:
    ...
    if PredictionDetailReservedKeys.DELETE in inbound_keyboard.info:
        if ReservedKeyboardKeys.CONFIRM not in inbound_keyboard.info:
            ...
            inline_keyboard = [get_yes_no_keyboard(user, inbound_keyboard=inbound_keyboard, add_inbound_key_info=True)]

            ot_text = phrases.PREDICTION_DELETE_CONFIRMATION.format(get_remaining_duration(
                user.prediction_creation_cooldown_end_date))
            await full_message_send(context, ot_text, update=update, keyboard=inline_keyboard,
                                    inbound_keyboard=inbound_keyboard, add_back_button=False)
            return
        else:
    ...
    if PredictionDetailReservedKeys.OPEN in inbound_keyboard.info:
        if prediction.get_status() is not PredictionStatus.NEW:
            await full_message_send(context, phrases.PREDICTION_ALREADY_OPEN, update=update, show_alert=True)
        elif ReservedKeyboardKeys.CONFIRM not in inbound_keyboard.info:
            # Request confirmation
            inline_keyboard = [get_yes_no_keyboard(user, inbound_keyboard=inbound_keyboard, add_inbound_key_info=True)]

            ot_text = phrases.PREDICTION_OPEN_CONFIRMATION.format(get_remaining_duration(
                user.prediction_creation_cooldown_end_date))
            await full_message_send(context, ot_text, update=update, keyboard=inline_keyboard,
                                    inbound_keyboard=inbound_keyboard, add_back_button=False)
            return
            ...
            if is_confirmed:
                ...
                await full_message_send(context, phrases.PREDICTION_OPEN_SUCCESS, update=update, show_alert=True)

    # Close prediction
    if PredictionDetailReservedKeys.CLOSE in inbound_keyboard.info:
        if prediction.get_status() is not PredictionStatus.SENT:
            await full_message_send(context, phrases.PREDICTION_ALREADY_CLOSED, update=update, show_alert=True)
        elif ReservedKeyboardKeys.CONFIRM not in inbound_keyboard.info:
            # Request confirmation
            inline_keyboard = [get_yes_no_keyboard(user, inbound_keyboard=inbound_keyboard, add_inbound_key_info=True)]

            ot_text = phrases.PREDICTION_CLOSE_CONFIRMATION.format(get_remaining_duration(
                user.prediction_creation_cooldown_end_date))
            await full_message_send(context, ot_text, update=update, keyboard=inline_keyboard,
                                    inbound_keyboard=inbound_keyboard, add_back_button=False)
            return
            ...
            if is_confirmed:
                ...
                await full_message_send(context, phrases.PREDICTION_CLOSE_SUCCESS, update=update, show_alert=True)
    ...
    if prediction.creator == user:
        ...
        if prediction.get_status() is PredictionStatus.NEW:
            ...
            inline_keyboard.append([Keyboard(phrases.KEY_OPEN,
        ...
        if prediction.get_status() is PredictionStatus.SENT:
            ...
            inline_keyboard.append([Keyboard(phrases.KEY_CLOSE,
                                             screen=Screen.PVT_PREDICTION_DETAIL,
                                             info=button_info)])
        ...
        if prediction.get_status() is PredictionStatus.SENT:
            button_info = {PredictionDetailReservedKeys.PREDICTION_ID: prediction.id}
            inline_keyboard.append([Keyboard(phrases.KEY_SEND_TO_GROUP,
        ...
        if prediction.get_status() is PredictionStatus.BETS_CLOSED:
            button_info = {PredictionDetailReservedKeys.PREDICTION_ID: prediction.id}
            inline_keyboard.append([Keyboard(phrases.KEY_SET_RESULT,
        ...
        inline_keyboard.append([Keyboard(phrases.PVT_KEY_PREDICTION_DETAIL_EDIT,
        ...
        inline_keyboard.append([Keyboard(phrases.KEYBOARD_OPTION_DELETE,
                                         screen=Screen.PVT_PREDICTION_DETAIL,
                                         info=button_info)])
        ...
        if prediction.get_status() is PredictionStatus.SENT:
            inline_keyboard.append(
                [Keyboard(phrases.KEY_SHARE, switch_inline_query=get_share_text(context, user, prediction))])
