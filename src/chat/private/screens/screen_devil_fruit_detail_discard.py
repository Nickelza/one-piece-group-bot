...
async def manage(update: Update, context: ContextTypes.DEFAULT_TYPE, inbound_keyboard: Keyboard, user: User) -> None:
    ...
    # Get devil fruit
    devil_fruit: DevilFruit = DevilFruit.get_by_id(inbound_keyboard.info[DevilFruitDiscardReservedKeys.DEVIL_FRUIT_ID])
    ...
    if ReservedKeyboardKeys.CONFIRM not in inbound_keyboard.info:
        ...
        ot_text = phrases.DEVIL_FRUIT_DISCARD_CONFIRMATION_REQUEST.format(devil_fruit.get_full_name())
        inline_keyboard: list[list[Keyboard]] = [
            get_yes_no_keyboard(user, screen=Screen.PVT_DEVIL_FRUIT_DETAIL_DISCARD, primary_key=devil_fruit.id,
