...
def get_play_amounts(current_bounty: int, win_odds) -> tuple[int, int, int, int]:
    ...
    return (
        final_bounty_if_win - current_bounty,
        current_bounty - final_bounty_if_lose,
        final_bounty_if_win,
...
def get_win_odd(user: User) -> float:
    ...
    return (
        Env.DOC_Q_GAME_WIN_ODD_FINAL_LOCATION.get_float()
...
async def validate_play(
    ...
    if user.bounty < Env.DOC_Q_GAME_REQUIRED_BOUNTY.get_float():
        ot_text = phrases.DOC_Q_GAME_NOT_ENOUGH_BOUNTY.format(
            get_belly_formatted(Env.DOC_Q_GAME_REQUIRED_BOUNTY.get_int()),
    ...
    previous_games: list[DocQGame] = DocQGame.select().where(
        (DocQGame.user == user) & (DocQGame.status == GameStatus.IN_PROGRESS)
    )
...
async def play_request(
    ...
    if await validate_play(update, context, user):
        ...
        doc_q_game.correct_choices_index = c.STANDARD_SPLIT_CHAR.join(
            str(i) for i in correct_choices_index
        )
        ...
        # Send media
        caption = phrases.DOC_Q_GAME_START.format(
...
async def keyboard_interaction(
    ...
    doc_q_game: DocQGame = DocQGame.get_or_none(
        DocQGame.id == keyboard.info[DocQReservedKeys.DOC_Q_ID]
    )
    ...
    if await validate_play(update, context, user, doc_q_game):
        ...
        if str(keyboard.info[DocQReservedKeys.CHOICE_INDEX]) in correct_choices_index:
            ...
            ot_text = phrases.DOC_Q_GAME_WIN.format(
                mention_markdown_v2(user.tg_user_id, user.tg_first_name),
            ...
            ot_text = phrases.DOC_Q_GAME_LOSE.format(
                mention_markdown_v2(user.tg_user_id, user.tg_first_name),
                correct_apple,
