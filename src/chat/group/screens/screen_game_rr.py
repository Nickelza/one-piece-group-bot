...
async def manage(
    ...
    if inbound_keyboard.screen == Screen.GRP_RUSSIAN_ROULETTE_GAME:
        ...
        await full_message_send(
            context,
            get_choice_text(russian_roulette.bullet_is_fired()),
            update=update,
            answer_callback=True,
        )
        ...
        for chamber_index, chamber in enumerate(row):
            ...
            if russian_roulette.is_chamber_center(row_index, chamber_index):
                ...
                keyboard_line.append(
                    Keyboard(emoji, info=button_info, screen=Screen.GRP_RUSSIAN_ROULETTE_GAME)
                )
...
def get_specific_text(game: Game, russian_roulette: RussianRoulette) -> str:
    ...
    return game_service.get_text(
        game,
        russian_roulette.is_finished(),
        russian_roulette.get_outcome(),
